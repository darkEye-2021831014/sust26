 ...
 * - Always prints the error to console.error AND the structured logger
 *   so the failure is visible in the dev terminal, not just in JSON logs.
 */
...
export function handleError(err: unknown): {
  ...
  if (err instanceof AppError) {
    ...
    // Also print to console so it shows up in the dev terminal.
    // eslint-disable-next-line no-console
    console.error(`[error] ${err.code} ${err.statusCode} — ${err.message}`);
    return {
  ...
  if (err instanceof ZodError) {
    ...
    // eslint-disable-next-line no-console
    console.error(`[error] VALIDATION_FAILED 400 — ${JSON.stringify(details)}`);
  ...
  if (err instanceof SyntaxError) {
    ...
    // eslint-disable-next-line no-console
    console.error(`[error] BAD_JSON 400 — ${err.message}`);
    return {
  ...
  // Print to console with the full stack so the dev can see it in the terminal.
  // eslint-disable-next-line no-console
  console.error('[error] INTERNAL_ERROR 500 — unhandled exception:');
  // eslint-disable-next-line no-console
  console.error(err);
  return {
